...
namespace Flub.Utils.Json
{
    ...
    public class JsonDictionaryConverter : JsonConverterFactory
    {
        ...
        {
            Type dictionaryType = (IsIDictionary(type) ? type : type.GetInterfaces().FirstOrDefault(IsIDictionary));
            if (dictionaryType == null)
                throw new ArgumentException($"Type of '{nameof(type)}' is not an dictionary.", nameof(type));
            return dictionaryType.GenericTypeArguments.Take(2);
        }
